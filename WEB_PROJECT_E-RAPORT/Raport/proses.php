	...
	switch($proses){
		...
		case "raport_insert":
		{
			...
			while($dtmp=mysqli_fetch_array($qtmp))
			{
				...
				'$dtmp[nilai_huruf]')";
				mysqli_query($raport_detail);
				
			}	
			//hapus data temp_raport_detil
			mysqli_query("DELETE FROM temp_raport_detail WHERE raport_id='$_POST[raport_id]'");
		...
		case "kelas_update":
			{
				$nama_tabel="kelas";
				$values="kelas_id='$_POST[kelas_Kode]', kelas_nama='$_POST[nmkelas]', jurusan_nama='$_POST[nmjurusan]'";
				$kondisi="inc='$_POST[inc]'";
				$hal="data_kelas";
				update($nama_tabel,$values,$kondisi);
				break;
